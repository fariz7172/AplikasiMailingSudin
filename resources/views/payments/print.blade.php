     ...
     @if($type === 'all' || $type === 'spp' || $type === 'sptjm_ls')
        ...
        <div class="print-area page-no-override type-kontrak {{ $type === 'kontrak' ? '' : 'page-break' }} font-serif">
            ...
            <div class="grid gap-y-1 text-[10pt] leading-tight text-left" style="grid-template-columns: 30px 230px 10px 1fr;">
                ...
                <span></span><span>Nomor Addendum I</span><span>:</span><span contenteditable="true" data-eid="48">{{ $payment->contract?->addendum_kontrak ?? '-' }}@if(!empty($payment->contract?->addendum_kontrak) && $payment->contract?->addendum_kontrak !== '0' && $payment->contract?->addendum_kontrak !== '-' && $payment->contract?->tgl_addendum) Tgl. {{ $payment->contract->tgl_addendum->locale('id')->translatedFormat('j F Y') }}@endif</span>
                <span></span><span>Nomor Addendum II</span><span>:</span><span contenteditable="true" data-eid="49">{{ $payment->contract?->addendum_kontrak2 ?? '-' }}@if(!empty($payment->contract?->addendum_kontrak2) && $payment->contract?->addendum_kontrak2 !== '0' && $payment->contract?->addendum_kontrak2 !== '-' && $payment->contract?->tgl_addendum2) Tgl. {{ $payment->contract->tgl_addendum2->locale('id')->translatedFormat('j F Y') }}@endif</span>
